...
export async function getProductosByProveedorApi(idproveedor) {
  try {
    const ProveedorFilter = `proveedor=${idproveedor}`;
    const url = `${BASE_API}/api/productos/?${ProveedorFilter}&necesita_Stock=true`;
    const response = await fetch(url);
    const result = await response.json();
    return result;
  } catch (error) {
